...
src/
  ...
  content.config.ts    # Content Collections schema (Astro v7 location)
...
Defined in `src/content.config.ts` using the Astro v7 Content Layer API with `glob()` loader. Posts are in `src/data/post/` as `.md` or `.mdx` files.
